...
def train(arg, train_loader, model: SiamRPN, loss_func: RPN_Loss, optimizer, epoch, vis: Vis, anchors):
    for batch_idx, data in enumerate(train_loader):
        ...
        z, u, cls_target, cls_mask, reg_target, reg_mask = data

        z = z.to(arg.device).unsqueeze(0)
        u = u.to(arg.device).unsqueeze(0)
        cls_target = cls_target.to(arg.device)
        cls_mask = cls_mask.to(arg.device)
        reg_target = reg_target.to(arg.device)
        reg_mask = reg_mask.to(arg.device)

        optimizer.zero_grad()
        model.template(z)
        cls_predict_m, reg_predict_m = model.forward(u)

        cls_predict_l = cls_predict_m.permute(1, 2, 3, 0).view(2, -1)
        cls_predict_l = cls_predict_l.permute(1, 0).contiguous()

        reg_predict_l = reg_predict_m.permute(1, 2, 3, 0).view(4, -1)
        ...
        vis.line(win='train', name='loss', x=aidx, y=loss.item())
        vis.line(win='train', name='c_loss', x=aidx, y=c_loss.item())
        vis.line(win='train', name='r_loss', x=aidx, y=r_loss.item())
        # print('-----------------------------')
        # print(loss.item(), c_loss.item(), r_loss.item())

        z_show = im_to_np_255(z[0])
        u_show = im_to_np_255(u[0])
        ...
        cls_target_show = cls_target.view(5, 17, 17).cpu().data.numpy()
        for i in range(5):
        ...
        c_tar_np = cls_target.cpu().data.numpy()
        ...
        reg_t = reg_target * reg_mask.unsqueeze(1).type(torch.cuda.FloatTensor)
...
def main():
    ...
    pre_train_model_path = '../model/snapshot_epoch20_model'
    ...
    for epoch in range(1, arg.train.epochs + 1):
        train(arg, train_data_set, model, loss_func, optimizer, epoch, vis, anchors)
