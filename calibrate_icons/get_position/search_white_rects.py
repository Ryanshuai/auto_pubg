...
from screen_parameter import min_gun_name_high, max_gun_name_high, min_gun_name_width, max_gun_name_width
...
if __name__ == '__main__':
    ...
    im = cv2.imread('screens_weapon/0.png')
    bboxes = search_white_size(im, min_gun_name_high, max_gun_name_high, min_gun_name_width, max_gun_name_width)
    shield = np.zeros_like(im, dtype=np.uint8)
    print('weapon?name')
    for rect in bboxes:
        y1, x1, y2, x2 = rect
        shield[y1:y2, x1:x2] = 1.0
        print([y1-3, x1-3, y2, x2])
    cv2.imshow('shield', im * shield)
    cv2.waitKey()
